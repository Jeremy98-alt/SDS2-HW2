...
The ACFs follow good behaviours, because we need to have samples independent each other during new iterations. Here, we see that the correlation are going further from the first lag, this is strictly decreasing. So, this is a good point!

For estimating the error of our hyper parameters, we can consider initially the formula of the variance MCMC, not in the case of MC, because the formula is sligthly different and because with the variance of MC we consider the samples as independent. 
...
Is important to denote that we can consider also a particular tool to estimate better the approximation error in order to taking into account positive and possible autocorrelations between the samples that we sampled before. 

In this case we consider the Monte Carlo Standard Error (MCSE) that is an estimate of the inaccuracy of MC samples for the expectation of posterior samples.
...
We can say again that $\alpha$ has the lower MCSE and gives a higher error respect other ones.
...
We can say also here that $\alpha$ has the higher posterior uncertainty.
...
The second prediction gives less precision than the previous. Due, to the approximation errors (MCSE) of the corresponding cases, but you can also see by the histograms plotted before.
...
```{r warning=FALSE, echo=FALSE, fig.align="center"}
tpm_graph <- graph_from_adjacency_matrix(tpm, weighted = TRUE)
plot(tpm_graph, edge.label = round(E(tpm_graph)$weight, 3), layout = layout.auto, cex=0.8, main="Markov Chain", vertex.size=25, edge.curverd=.1, arrow.size=.2, arrow.width=.1, edge.arrow.size=.8, vertex.label.cex=.9, vertex.color = "green")
```
...
As mentioned in the title, we try to make the simulations of the Markov Chain with the distribution described in this test, part 2. 

We iterate the markov chain 1000 consecutive times as follows, starting at state 1:
...
As mentioned in this section, we put below the relative frenquencies:
...
In order to try to formalize this part, we can say that the Markov Chain admits the *ergodic property* and means that this brings other properties i.e. the irreducibility and aperiodicity properties. Here there are the following definitions: 
...
point *b* approximates the stationary distribution $\pi$, given the ergodic property that says $\mathbb{P}_{X_0}^{(t)}(\cdot) \approx \pi(\cdot)$. Instead, point *c* assumes that there are 500 indipendent markov chains in which we have at the end a state $\pi_i$ considering the whole indipendent chains we will have the final probability that converges to $\pi$. 

n.b: This is possibile, from the fact that there is a probability to have a particular state $\pi_i$ through all indipendent markov chains (this gives the relative frequencies) multiplied by the total number of indipendent markov chains. This gives at the end the exact probability state $\pi_i$. 
...
hist(dt_fn_freq[, 1], main = "Behaviour of 500 simulations of Markov Chain with t =  1000 iterations", col = randomcoloR::randomColor(), xlab = "Empirical Frequency of state 1")
cat(paste("The mean relative frequency for the state 1 is exactly: ", mean(dt_fn_freq[, 1]), "\n", sep=""))

hist(dt_fn_freq[, 2], main = "Behaviour of 500 simulations of Markov Chain with t =  1000 iterations", col = randomcoloR::randomColor(), xlab = "Empirical Frequency of state 2")
cat(paste("The mean relative frequency for the state 2 is exactly: ", mean(dt_fn_freq[, 2]), "\n", sep=""))

hist(dt_fn_freq[, 3], main = "Behaviour of 500 simulations of Markov Chain with t =  1000 iterations", col = randomcoloR::randomColor(), xlab = "Empirical Frequency of state 3")
cat(paste("The mean relative frequency for the state 3 is exactly: ", mean(dt_fn_freq[, 3]), "\n", sep=""))
```

$$~$$

The stationary distribution is supported by the definition of the ***invariant distribution*** in the discrete case, we will see in the next section the definition of it in order to calculate this distribution.
...
The question of the steady state is: *"is there a vector of storing these probabilities?"*
The vector is called traditionally as vector of probabilities $\pi = [\pi_1, \pi_2, \pi_3]$, this vector takes into account the probability of each state.

The $\sum_{j=1}^{d} \pi_j = 1 \, \wedge \forall \pi_j \geqslant 0$, so if I have $\pi_1 = 0.5, \pi_2 = 0.5 \,\text{and}\, \pi_3 = 0$ means that I'm in the state $\pi_1$ or $\pi_2$ with that corresponding probabilities.

From the fact that, considering the transiction probability matrix I'm aware to know in a future where I could be through different iterations of the markov chain. 

My goal is to maintain steady the same vector of probabilities, in order to have the distribution forever over many times.
...
But, directly we can calculate and re-written this vector of probabilities using the transiction probability matrix as follows:
...
the $\pi = [\pi, ...., \pi_d]$ is also the ***eigenvector*** associated with $\lambda$. Reporting the previous concepts is important to have this condition of: $\sum_{j=1}^{d} \pi_j = 1 \, \wedge \forall \pi_j \geqslant 0$ to have the existence of the stationary distribution and we are interested to find this solution.

So, reporting the whole results we need to:
...
$$~$$
Is important to answer to this other question, interisting to know: *"the steady state could have 1 or more distributions?"* 

The answer is that there is the possibility to have different stationary distributions. Considering the fact that, we maybe could have different loops like this:

```{r echo=FALSE, fig.align="center"}
tpm2 <- matrix( c(0, 1/2, 1/2, 0,
                 0, 0, 1, 1,
                 0, 1, 0, 0,
                 0, 0, 0, 1), 
               nrow=4, byrow=T)
tpm_graph <- graph_from_adjacency_matrix(tpm2, weighted = TRUE)
plot(tpm_graph, edge.label = round(E(tpm_graph)$weight, 3), layout = layout.auto, cex=0.8, main="Markov Chain with more than one stationarity", vertex.size=25, edge.curverd=.1, arrow.size=.2, arrow.width=.1, edge.arrow.size=.8, vertex.label.cex=.9, vertex.color = "green")
```

$$~$$

The steady state *doesn't* tell us what happens to the markov chain as it progresses further and further in the future, this simple answers to the fact that if I start with markov chain with a particular distribution, this will be state there over times. 
...
The main behaviour of the markov chain will never chain due to the fact that the given distribution and the end and in the future will be steady to these values in fact:

$$~$$

```{r echo=FALSE}
niter <- 1000
relative_frequencies <- matrix(NA, ncol=niter+1, nrow=1)

out <- markovchainsampling(current_state = 2, tpm = tpm)
relative_frequencies[1, ] <- out$relativefrequencies

prop.table(table(relative_frequencies))
```
