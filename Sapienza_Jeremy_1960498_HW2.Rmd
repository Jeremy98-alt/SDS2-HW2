...
```{r echo=FALSE}
cat(paste("the average of tau at t iterations is: ", tail(mean_at_time_t$mean_tau, n=1), "\n", sep = "") )
cat(paste("the average of alpha at t iterations is: ", tail(mean_at_time_t$mean_alpha, n=1), "\n", sep = "") )
cat(paste("the average of beta at t iterations is: ", tail(mean_at_time_t$mean_beta, n=1), "\n", sep = "") )
cat(paste("the average of gamma at t iterations is: ", tail(mean_at_time_t$mean_gamma, n=1), "\n", sep = "") )
```
...
The ACFs follow good behaviours, because we need to have samples independent each other during new iterations. Here, we see that the correlation that going further from the first lag, this is strictly decreasing and this is a good point!
...
<!-- insert the igraph plot (?) -->
```{r warning=FALSE, echo=FALSE}
tpm_graph <- igraph::graph_from_adjacency_matrix(tpm, weighted = TRUE)
...
As shown in the title, we consider to simulate 500 times 1000 simulations of the Markov Chain:
...
We can show some simple plots and summaries of this previous step:
...
As we can see above, the behaviour is sligthly changed to the fact that we consider 500 times the simulations of 1000 iterations of a Markov Chain, we are going to focus on the steady state (also known as stationary state), in order to be able of making different sampling in that region that more likely is the best way to approximate the posterior region.

Now, we want to highlight the empirical mean of these simulations based on iteration of 500 times of our markov chain for each state:
...
$$~$$
Considering the Transiction probability matrix (TPM) mentioned before:
...
this matrix allows us to understand with how much probability I'm going to move from a state to another state, for example there is the probability of $\frac{1}{2}$ to go from the state $\pi_1$ to $\pi_2$ and so on.

The question of the steady state is: *"is there a vector of storing these probabilities?"*
The vector is called tradionally as vector of probabilities $\pi = [\pi_1, \pi_2, \pi_3]$, this vecto takes into account the probability of each state.
...
But, directly we can calculate this vector of probabilities using the transiction probability matrix as follows:
...
We have in the left side an invisible coefficient that we named as $\lambda = 1$ that is used in the general equation be the ***eigenvalue*** of the matrix, when it exists.

$$
(P^T-\lambda I)\pi = 0
$$

the $\pi = [\pi, ...., \pi_d]$ is also the ***eigenvector*** associated with $\lambda$. Reporting the previous concepts is important to have this condition of: $\sum_{j=1}^{d} \pi_j = 1 \, \wedge \forall \pi_j \geqslant 0$ to have the existence of the stationary distribution.

In the discrete case the steady state is easily computed as the probability of being in the state j after a single step, such as:

$$
\sum_{r=1}^{d}P_{rj}\pi_r = \pi_j
$$

Given the fact that the the vector of $[P_{1j}, ...., P_{rj}, ..., P_{dj}]$ is the vector of probabilities of being in j starting from all possible initial states.
...
tpm2 <- matrix( c(0, 1/2, 1/2, 0,
                 ...
                 0, 0, 0, 1), 
               nrow=4, byrow=T)
tpm_graph <- igraph::graph_from_adjacency_matrix(tpm2, weighted = TRUE)
