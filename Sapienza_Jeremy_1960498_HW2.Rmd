...
The main structure of our algorithm based on hybrid-kernels is structure based on the gibbs sampler algorithm. Considering the preliminary notation when $\theta = (\theta_1, ..., \theta_i, ..., \theta_k) \in \Theta \subset \mathbb{R}^k$ Given a target $\pi(\theta) = \pi(\theta_1, ..., \theta_k)$ the corresponding full conditionals will be denoted with:
...
for(t in 1:niter){
  ...
  gibbs_samples[t+1, "gamma"] <- notknown_fullcond_post_gamma_MH(y_obs, alpha = gibbs_samples[t+1, "alpha"], beta = gibbs_samples[t+1, "beta"], currentstate = gibbs_samples[t, "gamma"], x_ages = x_i, tau = gibbs_samples[t+1, "tau"], a = 1, greekpi = notknown_fullcond_post_gamma, nsims = 1000)
...
As we can see above, these are the relative frequency of the 500 simulated chains. The behaviour is sligthly changed to the fact that we consider 500 times the simulations of 1000 iterations of a Markov Chain. In this case, we are going to focus on the steady state (also known as ***stationary state***). Here we are able of making different samplings in that region that more likely is the best way to approximate the posterior region.
...
The vector is called tradionally as vector of probabilities $\pi = [\pi_1, \pi_2, \pi_3]$, this vector takes into account the probability of each state.
