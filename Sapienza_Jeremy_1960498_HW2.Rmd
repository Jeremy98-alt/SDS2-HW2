...
As mentioned in this section, we put above the relative frenquencies:
...
```{r}
markovchainsampling <- function(tpm, niter = 1000){
  # defining the number of iterations and the starting point
  current_state <- 1 
...
for(xT in 1:nchains){
  out <- markovchainsampling(tpm = tpm)
...
As we can see above, the behaviour is sligthly changed to the fact that we consider 500 times the simulations of 1000 iterations of a Markov Chain, we are going to focus on the steady state (also known as stationary state), we want to reach in a better into the stationary state.
...
The stationary distribution is supported by the definition of the ***invariant distribution*** in the discrete case, we highligthed the fact the:

- Given the transiction probability P
- Given the target distribution $\pi$ where a state $X_{0} \sim \pi$ belongs to

the invariant distribution is defined as: $\pi^{T} \cdot P = \pi \Leftrightarrow P^{T} \cdot \pi - I \cdot \pi = 0$. Multiplying the j-th column of the P with $\pi$, we have $\sum_{r=1}^{d}P_{r_{j}} \cdot \pi_{j} = \pi_{j} \,\,\,\, \forall j = {1, ..., d}$ this will bring you to be in an equilibrium condition.
